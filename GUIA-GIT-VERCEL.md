...
Paso a paso para trabajar el sitio desde tu PC con Git y publicarlo en Vercel.
...
se equivoca y rompe el deploy en Vercel.
...
| Visibilidad | **Private** (puedes cambiarlo después; Vercel funciona igual con repos privados en el plan gratuito) |
...
1. En **https://vercel.com** → *Sign Up* → **Continue with GitHub**.
2. Cuando te pida permisos sobre tus repositorios, dale acceso a `isanat`
   (o a todos, da igual).
3. Dentro de Vercel: **Add New… → Project**.
4. Busca `isanat` en la lista y dale **Import**.
5. En la pantalla de configuración, déjalo exactamente así:
   ...
   Vercel a veces intenta adivinar un framework. Si te propone algo como "Vite" o
   "Create React App", cámbialo a *Other*. Este sitio no tiene build: son archivos
   estáticos que se sirven tal cual.

6. **Deploy**. En menos de un minuto tienes una URL tipo `isanat.vercel.app`.

El archivo `vercel.json` del repo ya trae la cache de fuentes e imágenes, los
headers de seguridad y el redirect de `www` al dominio principal. No tienes que
tocar nada de eso en el panel.
...
Y ya. Vercel detecta el push y republica solo, en unos 30 segundos. No hay que
...
Vercel publica **esa rama en una URL de preview aparte**, sin tocar producción.
La encuentras en el panel, en *Deployments*. Se la puedes pasar al cliente para
que opine antes de que sea definitivo.
...
**Rollback en Vercel, sin tocar Git:** panel → *Deployments* → buscas un deploy
anterior que funcionaba → menú `⋯` → *Promote to Production*. Vuelve a esa versión
al instante. Útil si algo sale mal un viernes a las 7 de la tarde.
...
| En Vercel: 404 en la home | Los archivos quedaron dentro de una subcarpeta. Revisa el paso 1 o pon `isanat` en *Root Directory*. |
| En Vercel: se ve el HTML sin estilos ni logo | Mismo problema de rutas: `img/` y `fonts/` tienen que estar al lado de `index.html`. |
